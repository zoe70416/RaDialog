...
class RunnerBase:
    ...
    def train(self, wandb_run):
        ...
        #train iu 
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            custom_epochs = self.datasets['iu_cxr']['train'].custom_epochs_per_epoch if 'iu_cxr' in self.datasets else self.datasets['train'].custom_epochs_per_epoch
            for custom_epoch in range(custom_epochs):
                if 'iu_cxr' in self.datasets: #before first epoch
                    self.datasets['iu_cxr']['train'].set_custom_epoch(custom_epoch)
                else:
                    self.datasets['train'].set_custom_epoch(custom_epoch) #resorted after creating dataloader
                # training phase
                if not self.evaluate_only:
